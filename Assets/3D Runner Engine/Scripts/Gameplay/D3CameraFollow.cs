...
[System.Serializable]
public class D3CameraFollow : MonoBehaviour {
	...
	private bool shake;
    public static D3CameraFollow instace;
		...
		if(target != null){
            ...
            if (target.position.z >= 0)
            {
                if (shake == false)
                {
                    ...
                    {
                        posCamera.y = Mathf.Lerp(posCamera.y, 1 + height, 5 * Time.deltaTime);
                    }
                    posCamera.z = Mathf.Lerp(posCamera.z, target.position.z + distance, D3GameAttribute.gameAttribute.speed); //* Time.deltaTime);
    ...
    IEnumerator ShakeCamera(){
		...
		while(count <= 0.2f){
			...
			pos.x = transform.position.x + Random.Range(-0.01f,0.01f);
